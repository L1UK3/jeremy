...
Typed immutable specification for a crop's growth lifecycle.
...
- **`step_toward(fx: int, fy: int, tx: int, ty: int) -> str`**:
  Calculates a single cardinal step (`"NORTH"`, `"SOUTH"`, `"EAST"`, `"WEST"`) from `(fx, fy)` toward `(tx, ty)`. Resolves horizontal axis first. Returns `"PASS"` if already at destination.
...
- `empty -> bool`: `True` if tile contains no plant, weed, or animal structure.
- `is_plant -> bool`: `True` if `kind == "PLANT"`.
...
- `distance(x: int, y: int) -> int`: Manhattan distance to target coordinates.
- `age(current_day: int) -> int`: Days elapsed since planting.
- `is_ripe(current_day: int) -> bool`: Checks if crop age has reached maturity or ongoing harvest threshold.
- `is_unlocked(unlocked_quadrants: Iterable[str]) -> bool`: Validates whether tile resides in an unlocked quadrant.
...
- `empty_tiles(only_unlocked: bool = True) -> list[Tile]`: Returns empty tiles (`tile.empty == True`).
- `plants() -> list[Tile]`: Returns all plant tiles.
- `crops(crop: str) -> list[Tile]`: Returns plants matching specific crop string.
- `weeds(only_unlocked: bool = True) -> list[Tile]`: Returns weed tiles (`tile.is_weed == True`).
- `harvestable(crop: str | None = None) -> list[Tile]`: Returns mature plants and animal tiles with ready yields.
- `needs_water() -> list[Tile]`: Returns unwatered plant tiles (`watered == False`).
- `animals() -> list[Tile]`: Returns all pasture and coop tiles containing animals.
- `needs_feed() -> list[Tile]`: Returns animal tiles needing feed today.
- `needs_care() -> list[Tile]`: Returns animal tiles needing care today.
- `has_fertilizer_tiles() -> list[Tile]`: Returns animal tiles with ready-to-collect fertilizer.
...
- `nearest_shed(x: int, y: int) -> tuple[int, int]`: Returns nearest shed interaction coordinate.
