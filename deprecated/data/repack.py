...
try:
    if WORK.exists():
        for batch in pq.ParquetFile(WORK).iter_batches(batch_size=BATCH):
            tbl = batch.to_pydict()
            keep_i = [
                i for i, e in enumerate(tbl["episode_id"]) if e not in raw_ids
            ]
            if keep_i:
                writer.write_table(
                    pa.table(
                        {
                            "episode_id": [
                                tbl["episode_id"][i] for i in keep_i
                            ],
                            "replay_json": [
                                tbl["replay_json"][i] for i in keep_i
                            ],
                        },
                        schema=SCHEMA,
                    )
    ...
    for start in range(0, len(files), BATCH):
        ...
        writer.write_table(
            pa.table({"episode_id": ids, "replay_json": blobs}, schema=SCHEMA)
        )
finally:
    writer.close()
TMP.replace(WORK)
...
print(
    f"replays.parquet: {carried} carried + {len(files)} from raw = {total} rows, "
