    ...
    for seat in (0, 1):
        ...
        for t, step in enumerate(steps):
            ...
            hires_by_day[day] = max(
                hires_by_day.get(day, 0), farm["hires_today"]
            )
        ...
        if final and final > 0:
            cross = next(
                (t for t, m in enumerate(money) if m > final * 0.1), None
            )
...
def main():
    ...
    if (
        HASH_OUT.exists()
    ):  # the hash table fills in its own pass, so track it apart
        ...
        for eid, blob in zip(
            ...
            try:
                ...
                if eid not in have_hash:
                    new_hash_rows.extend(
                        dict(episode_id=eid, **r) for r in stream_hashes(replay)
                    )
    ...
    if new_hash_rows:
        ...
        h = (
            pd.concat([old_hash, h], ignore_index=True)
            if old_hash is not None
            else h
        )
        h = h.sort_values(["episode_id", "seat"])
        h = h[
            ["episode_id", "seat", "turns"] + [f"stream_h{t}" for t in PREFIXES]
        ]
        h.to_csv(HASH_OUT, index=False)
        print(
            f"stream_hashes.csv: {h.episode_id.nunique()} episodes, {len(h)} rows"
        )
    ...
    out = out[
        lead
        + sorted(plant_cols)
        + sorted(c for c in out.columns if c.startswith("price_"))
    ]
    out.to_csv(OUT, index=False)
    print(
        f"episode_features.csv: {out.episode_id.nunique()} episodes, {len(out)} rows, "
...
def write_daily_stats(here):
    ...
    lad = eps[
        eps.type.eq("EPISODE_TYPE_PUBLIC")
        & eps.state.eq("COMPLETED")
        & eps.winner_bank.gt(0)
    ].copy()
        ...
        rows.append(
            {
                "date": day,
                "ladder_games": len(g),
                "teams_active": pd.unique(
                    g[["team_0", "team_1"]].values.ravel()
                ).size,
                "median_winner_bank": round(g.winner_bank.median(), 1),
                "p90_winner_bank": round(g.winner_bank.quantile(0.9), 1),
                "record_bank": g.winner_bank.max(),
                "replay_coverage": round(
                    g.episode_id.isin(have_replay).mean(), 3
                ),
            }
        )
