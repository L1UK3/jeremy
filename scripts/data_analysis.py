...
print(f"First Land Day    : Day {TOP_PERFORMERS['first_land_day'].median():.0f}")
...
def get_top_eps(df, n=10):
    ...
    return df.sort_values("final_money", ascending=False).drop_duplicates("episode_id").head(n)
...
def main(n=10):
    ...
    for _, row in top_eps.iterrows():
        ...
        scanner = dataset.scanner(filter=pads.field("episode_id") == ep_id, batch_size=1)
        ...
        print(f"Loaded Episode {ep_id} (Score: ${score:,.2f}, {len(replay['steps'])} turns)")
...
if __name__ == "__main__":
    args = argparse.ArgumentParser(description="Analyze top scoring episodes and render replays.")
